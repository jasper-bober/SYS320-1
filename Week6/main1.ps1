...
while($operation){
    ...
    elseif($choice -eq 3){ 
        ...
        if (chechUser($name)) {
            Write-Host "Username already exists, exiting menu now" | Out-String
            return true 
        } else {
        return false
        }
        ...
        if (checkPassword($password)) {
            ...
            Write-Host "Password does not satisfy conditions"
            return false
        }
    }
}
    ...
    elseif($choice -eq 4){
        ...
        if (checkUser($name)) {
            ...
            Write-Host "User could not be found" | Out-String
        }
    }
    ...
    elseif($choice -eq 7){
        ...
        if (checkUser($name)) {
            $days = Read-Host -Prompt "Enter the number of days back"
    ...
    elseif($choice -eq 0){
        ...
        if ($atRiskUsers.Length -gt 0) {
            ...
            Write-Host "No at risk users could be found" | Out-String
        }
